...
- Apply the Traefik Middleware in the `authelia` namespace: `authelia/traefik.forwardauth.yml`.
- Reference it from any protected Ingress with:
...
- The middleware forwards to `http://authelia.authelia.svc.cluster.local/api/verify?rd=https://authelia.apps.k8s.enros.me` and exposes identity headers (`Remote-User`, `Remote-Name`, `Remote-Email`, `Remote-Groups`).
