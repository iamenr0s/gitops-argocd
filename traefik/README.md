...
- App manifest: `traefik/application.yml` references the chart and `traefik/values.yaml`.
...
- For bare‑metal or non‑cloud LB: set `service.externalIPs` in `traefik/values.yaml`.
...
- `traefik/values.yaml`: Helm values controlling Traefik.
...
- To prevent Traefik app health from being blocked by `LoadBalancer` provisioning delays, the service has:
  - `argocd.argoproj.io/ignore-healthcheck: "true"` under `service.annotations` in `traefik/values.yaml`.
...
- Logs:
  ...
  - Increase log verbosity in `values.yaml` with `logs.general.level: DEBUG`.

## Updating
- Adjust chart version in `traefik/application.yml` `targetRevision`.
- Modify `traefik/values.yaml` for ports, entrypoints, TLS, and providers.
